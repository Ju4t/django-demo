...
CACHES = {
    "default": {
        ...
        "LOCATION": "redis://%s:%s/0" % (env('KV_HOST'), env('KV_PORT', 6379)),
        "OPTIONS": {
    ...
    "cache": {
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://%s:%s/1" % (env('KV_HOST'), env('KV_PORT', 6379)),
        "OPTIONS": {
            "IGNORE_EXCEPTIONS": True,
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
            "CONNECTION_POOL_KWARGS": {"max_connections": 100},
...
# Celery settings
CELERY_BROKER_URL = 'amqp://%s:%s@%s' % (env('MQ_USER', 'guest'), env('MQ_PASS', 'guest'), env('MQ_HOST'))
# CELERY_RESULT_BACKEND = 'amqp://guest:guest@localhost:5672/'
# CELERY_RESULT_BACKEND = 'db+sqlite:///results.sqlite'
CELERY_RESULT_BACKEND = 'redis://%s:%s/1' % (env('KV_HOST'), env('KV_PORT', 6379))
